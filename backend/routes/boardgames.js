...
router.get('/', async (req, res) => {
    ...
    try {
        const result = await session.run('MATCH (n:BoardGame) RETURN n');
...
router.get('/:id', async (req, res) => {
    ...
    try {
        const result = await session.run(
            'MATCH (n:BoardGame {id: $id}) RETURN n',
            { id: req.params.id }
        );
...
// POST create board game
router.post('/', async (req, res) => {
    const session = getSession();
    try {
        const props = Object.keys(req.body).map(k => `${k}: $${k}`).join(', ');
        const query = `CREATE (n:BoardGame {${props}}) RETURN n`;
...
// PUT update board game
router.put('/:id', async (req, res) => {
    const session = getSession();
    try {
        const sets = Object.keys(req.body).map(k => `n.${k} = $${k}`).join(', ');
        const query = `MATCH (n:BoardGame {id: $id}) SET ${sets} RETURN n`;
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const result = await session.run(
            'MATCH (n:BoardGame {id: $id}) DETACH DELETE n RETURN count(n) AS deleted',
            { id: req.params.id }
        );
