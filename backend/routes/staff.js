...
router.get('/', async (req, res) => {
    ...
    try {
        const result = await session.run('MATCH (n:Staff) RETURN n');
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const query = `CREATE (n:Staff {${props}}) RETURN n`;
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        const query = `MATCH (n:Staff {id: $id}) SET ${sets} RETURN n`;
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const result = await session.run(
            'MATCH (n:Staff {id: $id}) DETACH DELETE n RETURN count(n) AS deleted',
            { id: req.params.id }
        );
