...
const entities = ['customers', 'cats', 'menu', 'boardgames', 'visits', 'orders', 'payments', 'staff', 'graph'];
...
router.post('/', async (req, res) => {
  ...
  try {
    const label = '${entity.charAt(0).toUpperCase() + entity.slice(1)}';
    if ('${entity}' === 'graph') return res.status(400).json({error: 'Invalid operation'});
...
`;
  fs.writeFileSync(path.join(routesDir, `${entity}.js`), code);
});
